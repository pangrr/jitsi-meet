...
// hack to modify interfaceConfig
// do not commit!
interfaceConfig.TOOLBAR_BUTTONS.push('polls');
...
MiddlewareRegistry.register(store => next => action => {
    switch (action.type) {
        ...
        case VOTE_POLL: {
            ...
            conference && conference.voteInPoll(choiceID);
            break;
        }
        case END_POLL: {
            const { conference } = store.getState()['features/base/conference'];

            conference && conference.endPoll();
            break;
        }
    }
    return next(action);
...
function _addPollMsgListener(conference, store) {
    ...
    console.log('_addPollMsgListener conference.on')
    conference.on(
    ...
    conference.on(
        ...
        (choices, poll, question) => {
            console.log('conference.on(JitsiConferenceEvents.POLL_STARTED')
